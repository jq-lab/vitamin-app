...
final class AccessibilityUITests: XCTestCase {
    ...
    func testPrimarySurfacesExposeAccessibleLabelsAndTouchTargets() {
        ...
        XCTAssertTrue(app.descendants(matching: .any)["cycle.energyDashboard.frame"].waitForExistence(timeout: 3))
        XCTAssertTrue(app.descendants(matching: .any)["cycle.energyCalendar.card"].waitForExistence(timeout: 3))
        XCTAssertFalse(app.otherElements["vitora.input.dock"].exists)
        XCTAssertFalse(app.textFields["vitora.input.text"].exists)
        assertMinimumTouchTarget(app.buttons["cycle.energyCalendar.toggle"], name: "Energy calendar toggle")
        assertMinimumTouchTarget(app.buttons["cycle.energyCalendar.day.30"], name: "Energy calendar selected day")
