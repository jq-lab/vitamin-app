...
- 指标卡：日历下方可见 `6.5 mmol/L`、更新时间、`88.0% TIR` 环和设备连接状态。
...
| 区域 | 必须通过 |
| --- | --- |
| Header | 左上头像进入我的/设置；右上分享生成周期卡片截图；头像与分享触控不小于 44pt；中间只保留轻标题，不放报告胶囊。 |
...
| 指标卡 | 存在 `cycle.energyMetric.card`；显示 `6.5 mmol/L`、更新时间、TIR 环和设备连接状态；不出现诊断或治疗判断。 |
