...
| F-P0-CYCLE-001 | Cycle 的心智是长期节律回顾，不是数据仪表盘。 | D-076 后 Cycle 顶部增加原生 SwiftUI `FlowerMapView` 花之地图模块，使用当前暖纸背景、二维字符串数组控制不规则菱形深圳轮廓、View/Text/Canvas 绘制花草和土地，不引入 RN/Expo、图片、SVG 地图或额外底部 Tab；v1 mock 城市为深圳 24/36、下一站广州，支持种下今天的花、点击空地种花、花朵详情、问号说明和城市节点切换，动效为 scale + pulse 与左侧详情浮层。D-084 后 Header 中间胶囊改为报告维度 `本周 / 趋势对比 / 近期`，默认本周；地图下方不再显示第二套文件夹 tab，只保留单张详情报告框，内容随 Header 胶囊切换；城市预览只由地图内深圳 / 广州节点和点状路线触发。`本周` 展示标题、状态摘要、7 日柱状图、三段说明、四格监测摘要和当前周期进度，`趋势（对比）` 展示月度综合对比、4 行蓝色进度条、归因解释和本月总结，`近期` 展示能量折线图、监测解释、下一步和黄色描边按钮；首屏移除可见 `周期回顾` 标题、能量动态卡、三张节律洞察卡和 `本周期花架证据`，不恢复日历首页、不引入打卡、连续天数、完成率、红点或任务清单；1536 展示图可用三张并排报告卡表达同一套报告语言，但不进入 app bundle。 | D-037, D-051, D-052, D-053, D-055, D-058, D-067, D-071, D-073, D-076, D-084 |
