...
| F-P0-CYCLE-001 | Cycle 的心智是长期节律回顾，不是数据仪表盘。 | D-093 后 Cycle 首屏从花田地图切换为能量复盘日历；D-095 后顶部合并为一个大圆角日历头卡：左侧我的/设置、右侧分享和折叠周历在同一容器内，不显示独立 `周期` 标题。折叠态每一天以大椭圆 pill + 内部小圆环显示当日能量，展开态显示整月所有日期的能量进度条；其下保留今日/本周能量摘要卡，主信息必须是今天能量和本周均值，`6.5` 只作为小参考 chip，不再做成大号 `6.5 mmol/L` 设备读数卡；`本周 / 趋势对比 / 近期` 报告入口移动到摘要卡下方，报告详情继续作为滚动后的解释层。该日历只表达能量复盘分布，不作为任务、打卡、完成率、连续天数、红点或周期管理日历；不引入 RN/Expo、图片、SVG、额外底部 Tab、花园手册或成长册。首屏仍移除可见 `周期回顾` 标题、旧能量动态卡、三张节律洞察卡和 `本周期花架证据`。 | D-037, D-051, D-052, D-053, D-055, D-058, D-067, D-071, D-073, D-076, D-084, D-093, D-094, D-095 |
...
| D-094 | F-P0-CYCLE-001 |
| D-095 | F-P0-CYCLE-001 |
