...
9. 确认 Today 首页不出现 `花园手册`、`查看成长卡`、`昨晚的薄荷芽发芽了`、`今晚可种` 或种子选择入口；智能监测首页卡也不展示白雏菊进度条，花朵进度只保留在提醒 Sheet 等二层反馈处。
10. 确认 Today 建议卡标题为 `智能监测`，左侧是低像素早上太阳；内嵌大卡顶部大标题为 `身体翻译器`，副文案为 `正在把今天翻译成管家方案`；首页卡内不出现 `开放状态 86 / 100`、白雏菊进度、动态数据胶囊、`推荐组合`、`休息 + 补剂` 重复标题或圆形加号；下方左侧是 `今日推送` 两张竖向扑克牌式行动卡，右侧是 `周期建议` 和 `黄体期 D18`；`换一换` 能刷新两侧内容；金黄色 `一键提醒` 打开提醒 Sheet，Sheet 顶部展示白雏菊进度条，时间按键支持上下滑动并可单选一个提醒；首屏不出现 `为什么` 按钮。
...
3. 确认 Cycle 首页没有日历预览、`30 天成长册`、花园手册、种子选择或任务完成心智；低权重 `本周期花架证据` 只展示最近建议反馈花卡。
