...
| 区域 | Cycle |
| 优先级 | P0 Swift MVP |
| 产品行为 | Cycle 首页展示长期节律回顾：顶部原生花之地图表达深圳到广州的恢复资源进度，地图下方用单张详情报告框解释 `本周 / 趋势对比 / 近期`。 |
| 用户价值 | 用户理解长期节律如何影响 Today，并看到 Vitora 从周期里学到了什么，而不是只看日历。 |
| 输入 | 周期上下文、今日状态、历史能量、睡眠/HRV 摘要、记录事件。 |
| 输出 | 花之地图、深圳 / 广州点状路线、花朵收集入口、单张详情报告框、二层详情入口、设置入口、分享入口。 |
...
| 验收标准 | 首页无日历预览；Header 胶囊为 `本周 / 趋势对比 / 近期` 并切换地图下方同一张详情报告框；花之地图使用原生 SwiftUI/Canvas 绘制；可长按问 Vitora；不出现成长册进度、打卡、完成率、连续天数或任务清单。 |
