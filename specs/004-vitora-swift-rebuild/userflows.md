...
| UF-003 | Today | 下拉揭示 Energy Ball | P0 Swift MVP | redesign | F-P0-ORB-001 |
...
### UF-003 · 下拉揭示 Energy Ball

**目标**：保留能量球的仪式感和品牌记忆，但不压住 Today 首页主结构。

**入口**：每天第一次打开 Today；用户在 Today 顶部下拉；晚间复盘。

**主路径**：

1. 默认 Today 不常驻大能量球。
2. 用户从顶部下拉，Energy Ball 从顶部弹性出现。
3. 未过阈值松手则收回。
4. 过第一阈值进入半展开：显示 68%、状态词、关键窗口、主要依据。
5. 继续下拉进入全屏 Energy Ritual：显示 Vitora 如何形成今日理解。
6. 向上滑或关闭回 Today。

**决策点**：是否继续下拉；是否进入全屏；是否问 Vitora；是否跳过。

**空状态 / 低数据路径**：低数据时显示“基于目前信息”的能量理解，不夸大确定性。

**退出状态**：Energy Ball 收起、半展开、全屏完成或进入 Vitora 解释。
...
6. 输入区上方显示快捷上下文 chips：周期、睡眠、营养、情绪、能量、+。
7. 底部 input dock 支持语音入口、文字输入、plus 和发送。
...
**入口**：晚间提醒；Vitora Tab；Today 已接受建议状态；用户主动复盘。
...
1. Cycle 顶部显示标题和 profile/settings 入口。
2. 第一卡 `当前周期阶段与今天` 显示 Day 18、黄体期中段、阶段轴、Vitora 今日建议已考虑此阶段。
3. 第二卡 `能量动态` 显示日/周/月切换、本周平均、趋势曲线和 Vitora 叙事摘要。
4. 用户单击任一卡进入二层详情。
5. 用户长按卡片打开 context menu：问 Vitora 为什么、告诉 Vitora 这里不准、查看详情。
