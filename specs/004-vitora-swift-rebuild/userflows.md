...
1. 用户在注册页选择 Apple / 微信 / QQ / 本地体验；协议未勾选时点击进入方式会同步确认协议并继续。
2. 登录后不再询问昵称，直接进入聊天式补充页。
3. Pixel Vitora 逐项提问，用户回答关注方向、恢复方式、周期清晰度。
4. 同一聊天页内选择绑定设备 / Apple 健康或先跳过。
5. 用户点击 `进入 Vitora`，直接进入 Today 低数据或设备增强路径。

**决策点**：选择哪种进入方式；周期信息是否清楚；HealthKit / 设备是否授权；是否跳过可选项。
...
**入口**：Today 首页能量碗本体或 `查看数据` 小入口；晚间复盘仍可使用能量对比表达。

**主路径**：

1. 用户进入 Today，首屏看到分层综合能量碗、单层高对比数字、状态词、`查看数据` 和无背景周期线轴，数据小标签不在首页出现，文字不压在碗体上。
2. 用户点击能量碗或 `查看数据`，进入同一个今日分析。
3. 用户先看到 `黄体期 Day 18` 和球形纹路报告，中间显示今日能量分数。
4. 今日分析解释睡眠、HRV、周期等关键监测项为什么影响今天的 68 分，并展示综合实时预测。
5. 预测内容下方直接给出今日推荐；睡眠、周期、HRV、心率只作为今日分析依据保留，不在 Today 首页作为预测切换入口。

**决策点**：是否查看今日分析；是否让 Vitora 校准。

**空状态 / 低数据路径**：低数据时显示“基于目前信息”的能量理解和可补充入口，不夸大确定性。

**退出状态**：用户停留 Today、进入今日分析或打开 Vitora contextual sheet。
